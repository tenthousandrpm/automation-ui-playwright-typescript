...
test.describe('Editor Validation', () => {
  test('submit button is disabled when title is empty', { tag: '@regression' }, async ({ authenticatedPage, editorPage }) => {
  ...
  test('submit button is disabled when body is empty', { tag: '@regression' }, async ({ authenticatedPage, editorPage }) => {
    await editorPage.goto();
